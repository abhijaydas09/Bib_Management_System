...
const handleParticipantSignup = async (e) => {
  ...
  try {
    const payload = {
      ...
      profilePic: profilePicUrl, // assuming you're sending base64, or send null
    };

    const response = await axios.post('http://localhost:8000/api/auth/player_signup', payload);

    console.log('Participant signup successful:', response.data);
    alert('Signup successful! Please login.');
    navigate('/login');
  } catch (err) {
    console.error('Participant signup failed:', err.response?.data || err.message);
...
const handleOrganiserSignup = async (e) => {
  ...
  try {
    const payload = {
      ...
      lastName: form.lastName,
      gender: form.gender,
      phoneNumber: form.phone, // align with participant
      email: form.email,
      password: form.password,
    ...
    const response = await axios.post('http://localhost:8000/api/auth/organiser_signup', payload);

    console.log('Organiser signup successful:', response.data);
    alert('Signup successful! Please login.');
    navigate('/login');
  } catch (err) {
    console.error('Organiser signup failed:', err.response?.data || err.message, err);
    alert(err.response?.data?.message || 'Signup failed.');
  }
};
  ...
  const organiserForm = (
    <form
      onSubmit={handleOrganiserSignup}
      style={{
        background: '#fff',
        borderRadius: 4,
        boxShadow: '0 4px 24px rgba(0,0,0,0.12), 0 1.5px 6px rgba(0,0,0,0.08)',
